...
class RMABInstance:
    ...
    description: dict
    offline_support_mask: np.ndarray | None = None
...
class LearningPolicy:
    def __init__(
        ...
        offline_prior_weight: float = 1.50,
        support_offline_leak: float = 0.0,
        support_trust_cap: float | None = None,
        support_local_bonus: float = 0.0,
        low_rank_blend_scale: float = 0.7,
        low_rank_projection_weight: float = 1.0,
        low_rank_local_bonus: float = 0.0,
    ) -> None:
        ...
        self.offline_support_mask = (
            instance.offline_support_mask if instance.offline_support_mask is not None else instance.support_mask
        )
        ...
        self.offline_prior_weight = offline_prior_weight
        self.support_offline_leak = support_offline_leak
        self.support_trust_cap = support_trust_cap
        self.support_local_bonus = support_local_bonus
        self.low_rank_blend_scale = low_rank_blend_scale
        self.low_rank_projection_weight = low_rank_projection_weight
        self.low_rank_local_bonus = low_rank_local_bonus

        base = np.full((self.n_arms, 2, self.n_states, self.n_states), 0.05)
        if self.support_prior:
            base = np.where(self.offline_support_mask, 0.20, 1e-6)
        if self.offline_prior:
            true_p = np.stack([instance.passive_p, instance.active_p], axis=1)
            noisy_prior = make_offline_transition_prior(true_p, self.offline_support_mask, self.support_prior)
            base += self.offline_prior_weight * noisy_prior
    ...
    def _sample_transition(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.threshold_prior:
            thresh = threshold_transition(
                p,
                threshold=0.025,
                support_mask=self.offline_support_mask if self.support_prior else None,
            )
            if self.gated_prior:
                p = gated_row_blend(
                    p,
        ...
        if self.low_rank_prior:
            ...
            if self.low_rank_projection_weight < 1.0:
                weight = np.clip(self.low_rank_projection_weight, 0.0, 1.0)
                low_rank = row_normalize((1.0 - weight) * p + weight * low_rank)
            if self.gated_prior:
                p = gated_row_blend(
                    p,
                    low_rank,
                    observed_rows,
                    self.low_rank_blend_scale * gate_scale,
                    rng=self.rng,
                    gate_mode=self.gate_mode,
                    beta_concentration=self.beta_gate_concentration,
                )
        ...
        if self.support_prior:
            if self.gated_prior:
                support_multiplier = np.where(self.offline_support_mask, 1.0, 0.05)
                p = row_normalize(p * support_multiplier)
            elif self.support_offline_leak > 0.0:
                support_multiplier = np.where(self.offline_support_mask, 1.0, self.support_offline_leak)
                p = row_normalize(p * support_multiplier)
            else:
                p = apply_support_mask(p, self.offline_support_mask)
    ...
    def select(
        ...
        if self.policy == "oracle":
            ...
            if self.policy == "tw":
                ...
                if self.support_prior and self.support_local_bonus > 0.0:
                    scores += self.support_local_bonus * zscore(local_scores)
                if self.low_rank_prior and self.low_rank_local_bonus > 0.0:
                    scores += self.low_rank_local_bonus * zscore(local_scores)
    ...
    def _transition_trust(self, observed_states: np.ndarray) -> np.ndarray:
        ...
        trust_cap = self.support_trust_cap if self.support_prior and self.support_trust_cap is not None else self.trust_cap
        return np.clip(trust, self.trust_floor, trust_cap)
